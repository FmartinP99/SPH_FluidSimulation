...
/*! \file
 *
 *   \brief C++ bindings for CL 1.0 (rev 48), CL 1.1 (rev 33),
 *       CL 1.2 (rev 15), CL 2.0 (rev 29), CL 2.1 (rev 17),
 *       and CL 2.2 (V2.2-11).
 *   \author Lee Howes and Bruce Merry
 *
 *   Derived from the CL 1.x C++ bindings written by
 ...
 *
 *       http://khronosgroup.github.io/CL-CLHPP/
 *
 *   The latest version of this header can be found on the GitHub releases page:
 *
 *       https://github.com/KhronosGroup/CL-CLHPP/releases
 *
 *   Bugs and patches can be submitted to the GitHub repository:
 *
 *       https://github.com/KhronosGroup/CL-CLHPP
 */

/*! \mainpage
 * \section intro Introduction
 * For many large applications C++ is the language of choice and so it seems
 * reasonable to define C++ bindings for CL.
 *
 ...
 * fixes in the new header as well as additional CL 2.0 features.
 ...
 * Due to the evolution of the underlying CL API the 2.0 C++ bindings
 * include an updated approach to defining supported feature versions
 * and the range of valid underlying CL runtime versions supported.
 *
 ...
 * decimal values representing CL runime versions. The default for
 * the target is 200, representing CL 2.0 and the minimum is also
 ...
 *
 * The CL 1.x versions of the C++ bindings included a size_t wrapper
 ...
 *
 * In CL 2.0 CL C is not entirely backward compatibility with
 * earlier versions. As a result a flag must be passed to the CL C
 * compiled to request CL 2.0 compilation of kernels with 1.2 as
 * the default in the absence of the flag.
 * In some cases the C++ bindings automatically compile code for ease.
 * For those cases the compilation defaults to CL C 2.0.
 ...
 *
 *   Defines the target CL runtime version to build the header
 *   against. Defaults to 200, representing CL 2.0.
 *
 ...
 *
 *   Enables device fission for CL 1.2 platforms.
 *
 ...
 *
 *   Default to CL C 1.2 compilation rather than CL C 2.0
 ...
 * \code
    ...
    int main(void)
    {
        ...
        for (auto &p : platforms) {
            ...
            if (platver.find("CL 2.") != std::string::npos) {
                plat = p;
            }
        }
        if (plat() == 0)  {
            std::cout << "No CL 2.0 platform found.";
            return -1;
        }
...
# pragma message("opencl.hpp: CL_HPP_TARGET_OPENCL_VERSION is not defined. It will default to 220 (CL 2.2)")
...
/* Forward target CL version to C headers if necessary */
...
namespace cl {
    namespace compatibility {
        ...
        *  CL C calls that require arrays of size_t values, whose
...
/*! \namespace cl
 *
 * \brief The CL C++ bindings are defined within this namespace.
 *
 */
namespace cl {
	class Memory;
...
#if CL_HPP_TARGET_OPENCL_VERSION >= 120
/**
 * CL 1.2 devices do have retain/release.
 */
		template <>
     ...
     */
		...
		/**
 * CL 1.1 devices do not have retain/release.
 */
template <>
struct ReferenceHandler<cl_device_id>
{
 ...
 */
    ...
    */
     ...
     *
     *  \param devices returns a vector of CL D3D10 devices found. The cl::Device
     *  values returned in devices can be used to identify a specific CL
     *  device. If \a devices argument is NULL, this argument is ignored.
     *
     *  \return One of the following values:
     *    - CL_SUCCESS if the function is executed successfully.
     *
     *  The application can query specific capabilities of the CL device(s)
...
#if defined(CL_USE_DEPRECATED_OPENCL_1_1_APIS)
	/**
 * Unload the CL compiler.
 * \note Deprecated for CL 1.2. Use Platform::unloadCompiler instead.
 */
...
#if defined (CL_HPP_USE_DX_INTEROP)
	/*! \brief Class interface for creating CL buffers from ID3D10Buffer's.
 *
 ...
 */
     ...
     */
			...
			// Run-time decision based on the actual platform
        {
            ...
            useCreateImage = (version >= 0x10002); // CL 1.2 or above
        }
    ...
    *              The channel order may differ as described in the CL
 ...
 *
 *  \see Memory
 *  \note Deprecated for CL 1.2. Please use ImageGL instead.
 */
 ...
 */
     ...
     * Valid for either CL >= 2.1 or when CL_HPP_USE_IL_KHR is defined.
     */
		Program(
     ...
     * \param context A valid CL context in which to construct the program.
     * \param devices A vector of CL device objects for which the program will be created.
     ...
     *   CL_INVALID_DEVICE if CL devices listed in devices are not in the list of devices associated with context.
     *   CL_INVALID_BINARY if an invalid program binary was encountered for any device. binaryStatus will return specific status for each device.
     *   CL_OUT_OF_HOST_MEMORY if there is a failure to allocate resources required by the CL implementation on the host.
     */
		Program(
				const Context& context,
				const vector<Device>& devices,
 ...
 */
    ...
    */
     ...
     */
				...
				cl_int* err = NULL)
		{
            ...
            {
                cl_uint version = detail::getContextPlatformVersion(context());
                useWithProperties = (version >= 0x20000); // CL 2.0 or above
            }
    ...
    */
     ...
     *     The pattern type must be an accepted CL data type.
...
#if CL_HPP_TARGET_OPENCL_VERSION >= 200
		/**
     * Enqueues a command that will release a coarse-grained SVM buffer back to the CL runtime.
     * This variant takes a raw SVM pointer.
     */
		template<typename T>
 ...
 * SVM buffer back to the CL runtime.
 * This variant takes a raw SVM pointer.
 */
	template<typename T>
